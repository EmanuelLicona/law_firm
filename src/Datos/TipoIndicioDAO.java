...
public class TipoIndicioDAO {
    ...
    public TipoIndicioDAO() {
        this.CON = Conexion.getInstancia();
    }
    
    public List<TipoIndicio> listar(String nombre){
        List<TipoIndicio> listar  = new ArrayList();
        try {
            insertando = CON.conectar().prepareCall("{call LISTAR_TIPO_INDICIO(?)}");
            insertando.setString(1, "%" + nombre + "%");
            rs = insertando.executeQuery();
            while (rs.next()) {
                listar.add(new TipoIndicio(rs.getInt(1),rs.getString(2),rs.getString(3)));
            }
            insertando.close();
            rs.close();
        } catch (Exception e) {
    ...
    public boolean insertar(TipoIndicio objeto) {
        respuesta = false;
        try {
            insertando = CON.conectar().prepareCall("{call MER_GUARDA_TIPOINDICIO(?,?,?,?,?)}");
    ...
    public boolean actualizar(TipoIndicio objeto){
        respuesta = false;
        try {
            insertando = CON.conectar().prepareCall("{call MER_GUARDA_TIPOINDICIO(?,?,?,?,?)}");
